...
def run_benchmark(pairs: list[dict]) -> list[dict]:
    """Run NLI entailment on all pairs."""
    results = []
...
def print_report(metrics: dict, total_pairs: int):
    print(f"\n--- NLI Entailment Benchmark Results ---")
...
if __name__ == "__main__":
    print(f"NLI entailment benchmark started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    pairs = load_all_pairs()
    print(f"Loaded {len(pairs)} pairs")
    print("Loading model and running inference (this takes a few minutes on CPU)...")

    results = run_benchmark(pairs)

    output_path = os.path.join(RESULTS_DIR, "nli_entailment_results.json")
    output = {
    ...
    metrics_path = os.path.join(RESULTS_DIR, "nli_entailment_metrics.json")
