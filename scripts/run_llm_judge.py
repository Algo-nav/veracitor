...
def stratified_sample(pairs: list[dict], n: int = 50) -> list[dict]:
    """
    Sample n pairs stratified by label.
    Allocates proportionally: ~20 clean, ~20 faithfulness, ~5 factuality, ~5 relevance.
    """
    random.seed(42)
    ...
    # Target counts per label (proportional to 40/40/10/10 distribution)
    targets = {
        "clean": 20,
        "faithfulness": 20,
        "factuality": 5,
...
def run_benchmark(pairs: list[dict]) -> list[dict]:
    """Run LLM judge on sampled pairs."""
    results = []
    ...
    for i, pair in enumerate(pairs):
        ...
        results.append({
            ...
            "label": pair["label"],
            "faithfulness_score": output.faithfulness.score,
            "factuality_score": output.factuality.score,
            "relevance_score": output.relevance.score,
...
if __name__ == "__main__":
    print(f"LLM judge benchmark started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    output_path = os.path.join(RESULTS_DIR, "llm_judge_results.json")
    output = {
        "method": "llm_judge",
        "model": "claude-sonnet-4-6",
        "run_at": datetime.now().isoformat(),
        "sample_size": len(results),
        "note": "Benchmarked on 50-pair stratified sample due to API cost constraints.",
        "results": results,
    }
    ...
    metrics_path = os.path.join(RESULTS_DIR, "llm_judge_metrics.json")
